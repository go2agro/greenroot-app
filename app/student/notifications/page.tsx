...
export default function StudentNotificationsPage() {
  ...
  useEffect(() => {
    ...
    async function loadPage() {
      ...
      setFirstName(studentProfile?.first_name?.trim() || '')
      setLastName(studentProfile?.last_name?.trim() || '')
  ...
  const filterCounts = useMemo(
    () => ({
      ...
      interview: notifications.filter((item) => item.category === 'interview').length,
  ...
  const handleFilterChange = async (tabKey: FilterKey) => {
    setActiveFilter(tabKey)

    if (tabKey === 'all' || tabKey === 'unread') return

    // Revalidate category-filtered list from server without blocking UI
    const { data } = await getNotificationsByCategory(tabKey)
    if (!data) return
  ...
  const handleNotificationClick = async (notification: NotificationItem) => {
    ...
    if (notification.related_id) {
      router.push(`/student/applications/${notification.related_id}`)
    }
  }
